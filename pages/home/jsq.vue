<template>
	<view class="content">
		<narBar :title="getLg('计算器')" @back="toBack()" ></narBar>
		<view class="" style="background: #fff;border-radius: 20rpx;
		margin-top: var(--status-bar-height);padding: 20rpx;width: 100%;margin: 15% 0;">
			<view class="view">
			...
			<view class="btns" @click="handleClick">
				<view v-for="item in btns" :key='item.text' :style="item.style" :class="item.className"
					:data-text="item.text"></view>
			</view>
		</view>
...
<script>
	...
	const numFun = (type, count) => {
		...
		// console.log(symbolReg2,symbolReg1);
		let num = undefined
		for (let i = 1; i <= count; i++) {
			// 乘除号下标, 3、7下标除、乘
			console.log(computes);
			const index = computes.findIndex(symbol => type === '0' ? symbolReg2.test(symbol) : symbolReg1.test(
				symbol))
			console.log('computes', computes);
			console.log(index, computes.length - 1);
			if (index == -1) return
			// 进行计算
			...
			handleClick({
				...
				switch (target.dataset.text) {
					...
					case KEYS[4 * 4 - 1]:
						...
						if (computes.length > 3) {
							...
							if (
								...
								// console.log('符号相同');
								this.viewValue = operation(computes[previousSymbolIndex - 1], computes[
									previousSymbolIndex + 1], computes[previousSymbolIndex])
								editStatus = 1
							}
					...
					case KEYS[KEYS.length - 1]:
						...
						console.log(cacheLastSymbol, cacheLastSymbol.length, 'cacheLastSymbol,cacheLastSymbol.length')
						if (cacheLastSymbol.length == 2) { // 第一次计算结束
							computes[0] = operation(computes[0], cacheLastSymbol[1], cacheLastSymbol[0])
						}
						console.log(computes, 'computescomputes')
					...
					default:
						...
						if (editStatus) {
							if (editStatus === 1) {
								const previousSymbolIndex = computes.length - 3
								computes.splice(previousSymbolIndex - 1, 3, operation(computes[previousSymbolIndex - 1],
									computes[previousSymbolIndex + 1], computes[previousSymbolIndex]))
							}
