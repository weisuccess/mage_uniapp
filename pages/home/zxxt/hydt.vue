<template>
	<view class="content">
		<narBar :title="'行业动态'" @back="toBack()" background="transparent" :rightTxt="'icon'" showRightBtn
			img="/static/products/message.png"></narBar>
		<scroll-view scroll-y="true"  :style="{height:windowHeight+'px',marginTop:statusBarHeight+'px'}" bindscrolltolower="onReachBottom">
...
<script>
	...
	export default {
		...
		methods: {
			getArr() {
				this.$request.get({
					url: "/app/knowBobInfo/industryDynamicsQuery",
					data: {
						id: this.id
					},
					success: rsp => {
