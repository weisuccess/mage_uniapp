				...
				<view class="flex-column-start-center" style="width:100vw;margin-top: 150rpx;" >
					<view class="" style="width:calc(100vw - 76px)">
						...
						<view class="flex-center-center mt40" style="">
							<view @click="toReg()" class="flex-center-center fz24" style="background-color:#f0f0f0 ;border-radius:50rpx;">
								<view style="background-color:#bf1609 ;color:#fff;padding: 15rpx 60rpx;border-radius: 50rpx;" class="py30">邮箱注册</view>
								<view style=" padding: 15rpx 60rpx;color:#000;">手机注册</view>
							</view>
						</view>
						...
						<view class="flex-between-center">
							...
							<input class="vinput" style="width:80%;padding-left:0rpx;font-size: 26rpx;color: #000;" type="text" placeholder-style="color:#a1a1a1" v-model="regInfo.email"
									 maxlength="30" placeholder="请输入邮箱" />
						...
						<view class="flex-between-center">
							...
							<input class="vinput" style="width:80%;padding-left:0rpx;font-size: 26rpx;color: #000;" type="text" placeholder-style="color:#a1a1a1" v-model="emailcode"
									 maxlength="4" placeholder="请输入验证码" />
									 
							<view @click="updateImageCode">
								<canvas style="width:220rpx;height:86rpx;" canvas-id="canvas" ></canvas>
							</view>
							<!-- <view style="background-color: #f01818; padding: 10rpx 30rpx;color: #fff;" >
								 发送
							</view> -->
							...
							<view class="flex-row-around-center mt30">
											<image src="/static/null/lock.png"	style="width: 30rpx;" mode="widthFix" />
										              <text class="ml20 fw600 fz26">输入密码</text>
							 </view>
						<view class="flex-between-center">
							
							<!-- <input class="vinput" style="width:90%;padding-left:0rpx;font-size: 26rpx;color: #000;" type="text" placeholder-style="color:#a1a1a1" v-model="regInfo.passWord"
					...
					<view class="fz26" @click="emailreg()" style="background-color: #cb2f2f;color:#fff;border-radius: 50rpx;padding: 20rpx 240rpx;margin-top: 80rpx;">
						注册
					</view>
	...
	export default {
		...
		methods: {
			...
			updateImageCode(){
				...
				console.log(this.mcaptcha.validate('aaaa'));
			},
			...
			changePh(e) {
									...
									emailreg(){
										...
										if (this.regInfo.email == null || this.regInfo.email=='') {
											uni.showToast({
												title: this.getLg("请输入邮箱"),
												icon: 'none',
												duration: 2000
											});
											return false;
										...
										if(!common.checkEmail(this.regInfo.email))
										{
											uni.showToast({
										...
										this.$request.post({
											url: "/app/dreamUserInfo/emailenroll",
											data: this.regInfo,
											success: rsp => {
												if (rsp.code == 200) {
													uni.showToast({
													...
													uni.setStorageSync('email',this.regInfo.email)
													
													uni.setStorageSync('password',this.regInfo.passWord)
													
													console.log(this.regInfo.passWord+"*****"+this.regInfo.email);
													
													setTimeout(() => {
														uni.redirectTo({
															url: '../login/newemaillogin'
														})
													}, 1000)
												} else {
													uni.showToast({
									...
									reg() {
										...
										if(!this.mcaptcha.validate(this.emailcode))
										{
											uni.showToast({
