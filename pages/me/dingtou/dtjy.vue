<template>
	<view class="content">
		<narBar :title="getLg('定投交易')" @back="toBack()"></narBar>
		...
		<view class="" style="background-color: #F1F1F1;">
			...
			<view class="box">
				<view class="flex-around-center lh100 text-align-c">
					...
					<view class="" style="width: 30%;">产品名称</view>
					...
					<view>
						<view style="padding: 40rpx 0;" v-for="(item,index) in list" :key="index">
							<view class="flex-around-center text-align-c">
								<view style="width: 20%;"
									:style="{'color':item.goodsType==1?'#00A00B':item.goodsType==3?'#00A00B':item.goodsType==2?'#D21112':''}">
									{{item.goodsType==1?'买入':item.goodsType==2?'到期':item.goodsType==3?'年化率':''}}
								</view>
								...
								<view style="width: 30%;" class="ellipsis color-666">{{item.investmentName}}</view>
								<view style="width: 30%;"
									:style="{'color':item.goodsType==1?'#00A00B':item.goodsType==3?'#00A00B':item.goodsType==2?'#D21112':''}">
									{{item.goodsType==1?'+':item.goodsType==3?'+':item.goodsType==2?'-':''}}
...
<script>
	...
	export default {
		...
		methods: {
			...
			getList() {
				this.$request.get({
					url: "/app/cashTreasureRegularInvestmentRecordingInfo/list",
					data: {
						pageNum: this.page,
						pageSize: this.size,
