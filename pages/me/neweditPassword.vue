<template>
	...
	<view class="content">
	     <narBarAndTop :background="'#fb4034'"  :title="'重置登录密码'"    @back="toBack()">
			 	...
			 	<view class="mt20" style="width: 100%;background: linear-gradient(to bottom, #fee9eb,  #ffffff, #ffffff); border-radius: 16rpx;">
					...
					<view class="mt50 px30" style="width:calc(100vw - 148rpx)">
						<view class="flex-between-center"> 
							  <text class="ml20 fw500 fz30">当前密码</text> 
						</view>
						<view class="flex-between-center mt10">
							
							<input class="" style="width:600rpx;padding:30rpx 20rpx;font-size: 28rpx;color: #000;" type="text" placeholder-style="color:#a1a1a1" v-model="params.password"
									 maxlength="11" placeholder="请输入当前登录密码" />
									 
						   <!-- <image src="/static/null/eye1.png"  style="width: 40rpx;" mode="widthFix" /> -->
						</view>
						<view class="flex-between-center mt20">
							  <text class="ml20 fw500 fz30">新密码</text> 
						</view>
						<view class="flex-between-center mt10">
							
							<input class="" style="width:600rpx;padding:30rpx 20rpx;font-size: 26rpx;color: #000;" type="text" placeholder-style="color:#a1a1a1" v-model="params.newPassword"
									 maxlength="11" placeholder="设置8位数字、字母、货币组合的新密码" />
									 
						   <!-- <image src="/static/null/eye1.png"  style="width: 40rpx;" mode="widthFix" /> -->
						</view>
						 
						<view class="flex-between-center mt20">
													  <text class="ml20 fw500 fz30">确认新密码</text> 
												</view>
												<view class="flex-between-center mt10">
													
													<input class="" style="width:600rpx;padding:30rpx 20rpx;font-size: 26rpx;color: #000;" type="text" placeholder-style="color:#a1a1a1" v-model="params.confirmNewPassword"
															 maxlength="11" placeholder="请再次输入新密码" />
...
<script>
	...
	export default {
		...
		methods: {
		 edit() {
		 				if (this.params.newPassword.length < 6) {
		 					uni.showToast({
		 						title: '密码至少6位',
		 						icon: 'none'
		 					})
		 				...
		 				if (this.params.newPassword != this.params.confirmNewPassword) {
		 					uni.showToast({
		 						title: '两次输入新密码不一样',
		 						icon: 'none'
		 					})
		 				...
		 				// const regex2 = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)[a-zA-Z\d]+$/;
		 				// if (!regex2.test(this.params.newPassword)) {
		 				// 	uni.showToast({
		 				// 		title: "密码得含有数字，大写字母和小写字母",
		 				// 		icon: 'none',
		 				// 		duration: 2000
		 				// 	});
		 				// 	return;
		 				...
		 				this.$request.get({
		 					url: '/app/dreamUserInfo/updateLoginPassword',
		 					data: this.params,
		 					success: rsp => {
		 						uni.showToast({
		 							title: rsp.message,
