<template>
	<view class="content">
		<narBar :title="getLg('充值记录')" @back="toBack()"></narBar>
		...
		<view class="" style="background-color: #F1F1F1;">
			...
			<view class="box">
				<view class="flex-around-center lh100 text-align-c">
					...
					<view class="" style="width: 30%;">渠道</view>
					...
					<view>
						<view style="padding: 40rpx 0;" v-for="(item,index) in list" :key="index">
							<view class="flex-around-center text-align-c">
								...
								<view style="width: 30%;" class="">{{item.tradeTitle}}</view>
								<view style="width: 30%;"
									:style="{'color':'#00A00B'}">
									+{{item.usdtMoeny}}
								</view>
							...
							<!-- <view class="flex-between-center">
								<view class="">
									<view class="lh80 fz30 fw700">{{item.tradeTitle}}</view>
									<view class="fz26" style="color: #838383;">交易时间：{{item.createTime | dateFormat}}
									</view>
								</view>
								<view
									style="display: flex;flex-direction: column;justify-content: flex-end;align-items: end;"
									:style="{'color':item.tradeStatus==2?'#00A00B':item.tradeStatus==3?'#D21112':''}">
									<view class="fw700 lh80">{{item.usdtMoeny}}USDT</view>
									<view class="fz26" style="">
										{{item.tradeStatus==1?'进行中':item.tradeStatus==2?'已完成':item.tradeStatus==3?'驳回':''}}
									</view>
								</view>
							</view>
							<image src="https://lanhu-oss.lanhuapp.com/286e64754ef16b426199bccc5fee9885"
								mode="widthFix">
							</image> -->
						</view>
					</view>
