	...
	<view>
		...
		<view :class="['content', show ? '' : 'contenthide']">
			<view class="title">
				<view class="close" @click="close()">
					<image src="https://cecece777.oss-cn-hongkong.aliyuncs.com/null/1737123126746close.png" />
				</view>
				<view class="center">请输入交易密码</view>
		...
		<view :class="['keyboard', show ? '' : 'active', isIphoneX ? 'isIphone' : '']">
			...
			<view class="keyboard-item delte" @tap="del()">
				<image class="img" src="https://cecece777.oss-cn-hongkong.aliyuncs.com/null/1737112701422del.png" mode="aspectFill" :lazy-load="true"></image>
			</view>
		</view>
...
<script>
	export default {
		...
		data() {
			return {
				password: '',
				shows: false, //用来判断金额是否为0，是0就让弹窗不显示并提示
				trantision: false
			};
		},
		created() {
		...
		watch: {
			show() {
				console.log(this.show, this.money);
				if (this.show == true) {
					this.password = ''
