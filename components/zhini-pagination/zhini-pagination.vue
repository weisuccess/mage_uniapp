<template>
	<view class="pageContain">
		...
		<view class="midClass">
			<view @click="handleCurrent(item)" class="item right-border" :class="[chooseIndex===item?'active':'']"
				v-for="(item,index) in pageArr" :key="index">{{item}}</view>
		</view>
