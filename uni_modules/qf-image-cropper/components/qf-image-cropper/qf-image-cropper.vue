<template>
	<view class="image-cropper" @wheel="cropper.mousewheel">
		...
		<view class="fixed-bottom safe-area-inset-bottom">
			...
			<block v-if="!choosable">
				<view class="rechoose" @click="back()">Hủy bỏ</view>
				<button class="button" size="mini" @click="cropClick">Chắc chắn</button>
			</block>
	...
	export default {
		...
		methods: {
			back(){
				...
				}else if(this.backType==4||this.backType==5){
					uni.navigateTo({
						url:'userCard2'
					})
				}
			...
			canvasToTempFilePath(canvas, canvasId) {
				...
				uni.canvasToTempFilePath({
					...
					success: async (res) => {
						// 生成的图片临时文件路径
						// #ifdef APP-PLUS
						let data = await this.detailImage(res.tempFilePath);
			...
			/** 确认裁剪 */
			cropClick() {
				uni.showLoading({ title: 'Cắt xén...', mask: true });
				if(!this.use2d) {
